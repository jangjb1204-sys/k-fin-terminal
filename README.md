...
한국어 기반 미국주식 금융 터미널입니다. 미국 주식, ETF, 지수, 금리, 원자재, 환율, 한국 주식, 뉴스, 차트, 포트폴리오, 주문 설계, AI 요약을 한 화면에서 다룹니다. 개인용 커스텀 앱이라 로그인 기능은 제거했고, 설정과 포트폴리오는 로컬 저장 방식으로 동작합니다.
...
- 단일 사용자: 로그인 없이 일반 시장 데이터, 차트, 뉴스, 포트폴리오, 리서치 상태, Paper 주문 화면 조회
- 로컬 저장: 포트폴리오, 기본 종목, AI/브로커 설정 저장
