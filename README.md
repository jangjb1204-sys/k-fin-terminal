...
한국어 기반 미국주식 금융 터미널입니다. 토스증권 Open API를 기본 데이터 소스로 사용해 미국 주식/ETF, 한국 주식, USD/KRW 환율, 캔들 차트, 보유자산 조회를 다룹니다. 개인용 커스텀 앱이라 로그인 기능은 제거했고, 옵션/주문 기능도 제외했습니다.
...
- 데이터: 토스증권 현재가, 캔들, USD/KRW 환율, 계좌 보유자산 읽기 전용 조회
...
- 포트폴리오: 수동 입력/수정, 토스 보유자산 읽기 전용 동기화 상태, 평가금액, 손익
...
보유자산 조회 계좌를 고정하려면 `TOSS_ACCOUNT_SEQ`를 추가합니다. 비워두면 앱이 `/api/v1/accounts`의 첫 계좌를 사용합니다.
